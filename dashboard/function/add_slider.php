...
//		echo $img_extension;
		$new_name = rand(1, 1000000000) . '.' . $img_extension; 
//		صوره جديده
		...
		if(! in_array($img_extension,$allowed_img)){
			...
			if($img_size > 15728640){
				...
				echo 'please check another file Losser than 5 MB';
				exit();
			}else{
    ...
    $stmt = $coon->prepare("INSERT INTO slider(slider_img,slider_title) VALUES (:img,:title)");
    $stmt->execute([
